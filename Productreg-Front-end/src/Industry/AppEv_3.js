...
import { Col, Row, Nav,Form,Modal ,Toast,Card, Image,Popover, OverlayTrigger, Button,InputGroup, Table, Dropdown, ProgressBar, Pagination, ButtonGroup } from '@themesberg/react-bootstrap';
...
export const App = () => {
    ...
    const [ShowsDefault, setShowsDefault] = useState(false);
    const handleClosess = () => setDefaultss(false);
    const handleCloses = () => setShowsDefault(false);
...
const loady=(id)=>{
  history.push('/industry/appfed/' +id);
}
  return (
    ...
    <Table responsive className="align-items-center table-flush">
        <thead className="thead-light">
          ...
          <th scope="col">#</th>
            ...
            <th scope="col">laboratory Analysis </th>
        ...
        {feeddata.map((item)=>(
                  ...
                  <td >{item.laboratory_analysis}</td>
                  <td >{item.recommendation}</td>
                  <td >{item.query}</td>
                  <td >{item.comments}</td>
                  <td>
                      <Button  onClick={()=>{loady(item.id)}} size="sm" variant ="tertiary" className="m-1">View Feedback</Button>
                  </td>
                  </tr>
              ))}
        </tbody>
  ...
  <Modal as={Modal.Dialog} centered show={ShowsDefault} onHide={handleCloses}>
       <Modal.Header>
      <Modal.Title className="h6">Application Details</Modal.Title>
    ...
    <Card border="light" className="shadow-sm mb-0 w=100 align-content-lg-center fmxw-1000">
        <Card.Body>
            ...
            <Col md={6} className="mb-3">
              <Form.Group id="firstName" name="old_password" className="mb-4">
                <Form.Label>Food additive</Form.Label>
                <Form.Control  placeholder={userdatass.food_additive} disabled />
              </Form.Group>
              <Form.Group id="firstName" name="old_password" className="mb-4">
                <Form.Label>Product image </Form.Label>
                <Form.Control  placeholder={userdatass.product_image} disabled />
              </Form.Group>
              <Form.Group id="firstName" name="old_password" className="mb-4">
                <Form.Label>Food ingredient image </Form.Label>
                <Form.Control  placeholder={userdatass.food_ingredient_image} disabled />
              </Form.Group>
              <Form.Group id="firstName" name="old_password" className="mb-4">
